...
$pdf->SetTitle('Relatório de Contas');
...
<h1>Relatório de Contas Totais</h1>';
...
while ($q = $res->fetch_assoc()) {
    $totalcontas++;
    $html .= '<tr>
        <td>' . htmlspecialchars($q['descricao_conta']) . '</td>
        <td>' . htmlspecialchars($q['valor']) . '</td>
        <td>' . htmlspecialchars($q['tipo']) . '</td>
        <td>' . htmlspecialchars($q['status_conta']) . '</td>
        <td>' . htmlspecialchars(date('d/m/Y H:i:s', strtotime($q['data_lancamento']))) . '</td>
        <td>' . htmlspecialchars(date('d/m/Y',strtotime($q['data_acerto']))) . '</td>
        <td>' . htmlspecialchars($q['forma_acerto']) . '</td>
    </tr>';
}

$html .= "<tr class='total'><td colspan='7'>Total de Contas</td><td>" . $totalcontas . "</td></tr>";
$html .= "</table>";

// Saída do PDF
$pdf->writeHTML($html, true, false, true, false, '');
$nome_arquivo = 'relatorio_contas_' . date('Ymd_His') . '.pdf';
